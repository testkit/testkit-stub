...
int HttpServer::getrequest(string requestbuf, struct HttpRequest *prequest)
{
	...
	if (prequest->path.find('?') == string::npos) {
		...
		if (content_index > -1) {
			prequest->content =
			    requestbuf.substr(content_index +
					      strlen("\r\n\r\n"));
		}
	} else {
...
void HttpServer::parse_json_str(string case_node)
{
	...
	if (parsed) {
		...
		m_current_block_index =
		    atoi(value["currentBlk"].asString().c_str());
...
void HttpServer::cancel_time_check()
{
	...
	if (timer_thread_info.thread_id.p) {
		...
		memset(&timer_thread_info.thread_id, 0,
		       sizeof(timer_thread_info.thread_id));
	}
#else
...
void HttpServer::set_timer(int timeout_value)
{
	...
	pthread_create(&timer_thread_info.thread_id, NULL, win_timer_callback,
		       &timer_thread_info);
#else
...
Json::Value HttpServer::splitContent(string content)
{
	...
	if (content != "") {
		std::vector < std::string > splitstr =
		    ComFun::split(content, "&");
		for (unsigned int i = 0; i < splitstr.size(); i++) {
			vector < string > resultkey =
			    ComFun::split(splitstr[i], "=");
...
void HttpServer::find_id(Json::Value paras, bool auto_test)
{
	...
	if (!found) {
		if (g_show_log)
			DBG_ONLY("[ Error: can't find any test case by id: " <<
				 id << " ]");
	}
	...
	while (1) {
		...
		if (content_length == 0) {
			...
			if (Content_Length != NULL) {
				char tmplen[32];
				sscanf(Content_Length, "%[^:]:%d", tmplen,
				       &content_length);
				if (content_length > 0) {
				...
				real_content_length += iDataNum;
		} else
			real_content_length += iDataNum;

		if (real_content_length >= content_length)
			break;
	}
	buffer[recvnum] = '\0';
	recvstr = buffer;
